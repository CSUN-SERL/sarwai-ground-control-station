...
namespace LiveFeedScreen.ROSBridgeLib
{
    public class ROSBridgeWebSocketConnection
    {
        ...
        private readonly List<Type> _publishers; //our publishers

        private readonly object _queueLock = new object();
        private string _serviceName;
        private Type _serviceResponse; // to deal with service responses
        private string _serviceValues;
        ...
        private static string GetMessageType(Type t)
        {
            return (string) t.GetMethod("GetMessageType",
                ...
                BindingFlags.FlattenHierarchy).Invoke(null, null);
        }
            ...
            if (s != null && !s.Equals(""))
            {
                ...
                if ("publish".Equals(op))
                {
                    ...
                    foreach (var p in _subscribers)
                        ...
                        {
                            Debug.Log("And will parse it " +
                                      GetMessageTopic(p));
                            var msg = ParseMessage(p, node["msg"]);
                            var newTask = new RenderTask(p, topic, msg);
                            lock (_queueLock)
                            {
                                var found = false;
                                for (var i = 0; i < _taskQ.Count; i++)
                ...
                {
                    Debug.Log("Got service response " + node);
                    _serviceName = node["service"];
                    _serviceValues = node["values"] == null
                        ? ""
                        : node["values"].ToString();
                }
                else
                {
        ...
        {
            private readonly ROSBridgeMsg _msg;
            private readonly Type _subscriber;
