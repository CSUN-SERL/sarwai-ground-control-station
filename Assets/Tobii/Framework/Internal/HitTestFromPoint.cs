...
namespace Tobii.Framework.Internal
{
    ...
    internal class HitTestFromPoint
    {
        ...
        public static bool FindObjectInWorld(out RaycastHit hitInfo,
            Vector2 point, Camera camera, float distance = Mathf.Infinity,
            int layerMask = 1)
        {
            var hitTestRay = camera.ScreenPointToRay(point);
            return Physics.Raycast(hitTestRay, out hitInfo, distance,
                layerMask);
        }
        ...
        /// <param name="layerMask">Optional: Which layers to hit test on.</param>
        /// <returns></returns>
        public static bool FindMultipleObjectsInWorldFromMultiplePoints(
            out IEnumerable<RaycastHit> hitInfos, IEnumerable<Vector2> points,
            Camera camera, float distance = Mathf.Infinity, int layerMask = 1)
        {
            var rays = CreateRaysFromPoints(points, camera);
        ...
        /// <returns>True if an object was found, false otherwise.</returns>
        public static bool FindObjectOnCanvas(out GameObject selectedObject,
            Vector2 point, int layerMask = -1, float minDepth = -Mathf.Infinity,
            float maxDepth = Mathf.Infinity)
        {
            var selectedCollider =
        ...
        /// <returns></returns>
        private static IEnumerable<Ray> CreateRaysFromPoints(
            IEnumerable<Vector2> points, Camera camera)
        {
    ...
    {
        public static IEnumerable<Vector2> CreateCircularAreaUniformPattern(
            Vector2 centralPoint, int radiusInScreenPixels, int numberOfPoints)
        {
            var points = new List<Vector2>();
            for (var i = 0; i < numberOfPoints; ++i)
            {
                var point = Random.insideUnitCircle * radiusInScreenPixels +
                            centralPoint;
                points.Add(point);
            }
        ...
        /// <returns></returns>
        public static Ray[] CreateCircleAroundCentralPoint(Vector2 centralPoint,
            Camera camera, int numberOfPoints, float patternRadius)
        {
            ...
            {
                var heading = Mathf.Deg2Rad * alpha;
                var offset = new Vector2(Mathf.Cos(heading) * patternRadius,
                    Mathf.Sin(heading) * patternRadius);
                positionsOfSecondRaysOnScreen[step] =
                    camera.ScreenPointToRay(centralPoint + offset);
                step++;
            }
