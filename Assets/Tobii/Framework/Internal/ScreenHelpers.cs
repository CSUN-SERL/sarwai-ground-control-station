...
namespace Assets.Tobii.Framework.Internal
{
    ...
    internal abstract class GameViewBoundsProvider
    {
        ...
        public IntPtr Hwnd
        {
            get
            {
                if (_hwnd == IntPtr.Zero) _hwnd = GetGameViewWindowHandle();
                return _hwnd;
            }
        ...
        protected virtual float GetToolbarHeight()
        {
            try
            {
                return
                    ...
                    2.0f; // seems only half the physica pixel size is returned by Unity, or that there are two stacked toolbars.
            }
        ...
        /// </summary>
        /// <returns></returns>
        protected virtual EditorWindow GetMainGameView()
        {
            var unityEditorType =
                Type.GetType("UnityEditor.GameView,UnityEditor");
            Debug.Assert(unityEditorType != null);
            var getMainGameViewMethod = unityEditorType.GetMethod(
                "GetMainGameView",
                BindingFlags.NonPublic | BindingFlags.Static);
            Debug.Assert(getMainGameViewMethod != null);
    ...
    internal class UnityPlayerGameViewBoundsProvider : GameViewBoundsProvider
    {
        public override Rect GetGameViewClientAreaNormalizedBounds()
        {
            ...
            var isTrueFullScreen = logicalWidth == Screen.width &&
                     ...
                     || logicalWidth < float.Epsilon))
            {
                ...
                var gameAspectRatio = (float) Screen.width / Screen.height;
                var logicalAspectRatio = (float) logicalWidth / logicalHeight;

                if (gameAspectRatio > logicalAspectRatio)
                {
                    //Bars to top and bottom
                    var gameHeightInLogicalPixels =
                        logicalWidth / gameAspectRatio;
                    return new Rect(0,
                        (logicalHeight - gameHeightInLogicalPixels) / 2.0f /
                        logicalHeight, 1,
                        gameHeightInLogicalPixels / logicalHeight);
                }

                //Bars to left and right
                var gameWidthInLogicalPixels = logicalHeight * gameAspectRatio;
                return new Rect(
                    (logicalWidth - gameWidthInLogicalPixels) / 2.0f /
                    logicalWidth, 0, gameWidthInLogicalPixels / logicalWidth,
                    1);
            }
    ...
    {
        private EditorWindow _gameWindow;
        ...
        public override Rect GetGameViewClientAreaNormalizedBounds()
        {
            ...
            var gameViewOffsetX =
                (gameWindowBounds.width - screenSize.x) / 2.0f;
            var gameViewOffsetY =
                (gameWindowBounds.height - screenSize.y) / 2.0f;
    ...
    internal class EditorGameViewBoundsProvider : GameViewBoundsProvider
    {
        ...
        public override Rect GetGameViewClientAreaNormalizedBounds()
        {
            ...
            return new Rect(0, toolbarHeight / (bottomRight.y - topLeft.y), 1,
                1 - toolbarHeight / (bottomRight.y - topLeft.y));
        }
        ...
        {
            return WindowHelpers.GetGameViewWindowHandle(Process
