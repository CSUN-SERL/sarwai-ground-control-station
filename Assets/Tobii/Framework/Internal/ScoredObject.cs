...
namespace Tobii.Framework.Internal
{
    ...
    {
        private static readonly GameObject EmptyGameObject =
            new GameObject("ScoredObject_EmptyGameObject");
        ...
        private readonly List<KeyValuePair<float, float>> _hits =
            new List<KeyValuePair<float, float>>();

        private readonly float _loseGazeDwellTime;

        public ScoredObject(GameObject gameObject,
            float gainGazeDwellTime = 0.05f, float loseGazeDwellTime = 0.15f)
        {
        ...
        {
            return IsRecentlyHit(Time.unscaledTime - _loseGazeDwellTime,
                Time.unscaledTime - _gainGazeDwellTime);
        }

        public bool IsRecentlyHit(float minimumTimestamp,
            float maximumTimestamp)
        {
            PruneOldHits(minimumTimestamp);
            var lastIndex =
                _hits.FindLastIndex(kvp => kvp.Key < maximumTimestamp);
            return lastIndex >= 0;
        }

        public float GetScore()
        {
            return GetScore(Time.unscaledTime - _loseGazeDwellTime,
                Time.unscaledTime - _gainGazeDwellTime);
        }
        ...
        public bool Equals(ScoredObject otherObject)
        {
            return GameObject.GetInstanceID() ==
                   otherObject.GameObject.GetInstanceID();
        }
